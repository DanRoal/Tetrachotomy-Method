...
def beta(z):
    return exp(.5*(custom_log(1 + amortiguado(z), cut)))

def G(z):
    return exp(1.0j*4*l_0*beta(z))

def beta_1(z):
    return (1/(2*l_0))*(pi*n_0 + (1/2)*custom_arg(G(z), cut) -
                        (1.0j/2)*log(abs(G(z)))) - beta(z)

tic = time.time()
polos = Encontrar_polos(beta_1, x_0=-10, x_1=10, y_0=-10, y_1=10, tol= 1e-2)
toc = time.time()
